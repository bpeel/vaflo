...
const N_STARS: u32 = 5;
const SAVE_STATE_KEY: &'static str = "shawffle-save-states";
...
impl Vaflo {
    ...
    fn update_title(&self) {
        if let Some(element) = self.context.document.get_element_by_id("title")
            .and_then(|c| c.dyn_into::<web_sys::HtmlElement>().ok())
        {
            let value = format!("Shawffle #{}", self.todays_puzzle + 1);
    ...
    fn set_lost_state(&mut self) {
        ...
        self.set_element_text(&self.swaps_remaining_message, "𐑑𐑵 𐑚𐑨𐑛 😔");
    }
    ...
    fn update_swaps_remaining(&self) {
        let text = if self.swaps_remaining == 1 {
            "1 swap left".to_string()
        } else {
            format!("{} swaps left", self.swaps_remaining)
        };
    ...
    fn share_results(&mut self) {
        ...
        match self.set_clipboard_text(&share_text) {
            Ok(()) => self.show_notice("Message copied to the clipboard"),
